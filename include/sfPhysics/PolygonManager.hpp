...
namespace sfp
{
	...
	class PolygonManager : public Polygon
	{
		...
		public:
			...
			inline int GetPointCount() {return Polygon::GetPointCount();}
			inline const sf::Vector2f& GetPoint(unsigned int index) {return Polygon::GetPoint(index);}
			
			// Funktionen von PolManager
			
			unsigned int GetConvexPolygonCount() {return myConvexPolygons.size();}
			const Polygon& GetConvexPolygon(unsigned int index) {return myConvexPolygons[index];}
			...
			const sf::Vector2f& GetPoint(unsigned int polygon, unsigned int index) {return myConvexPolygons[polygon].GetPoint(index);}
